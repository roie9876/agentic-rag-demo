...
def agentic_retrieval(agent_name: str, index_name: str, messages: list[dict]) -> str:
    ...
    """
    print(f"[agentic_retrieval] DEBUG: Starting agentic retrieval with agent_name='{agent_name}', index_name='{index_name}'")
    ...
    try:
        ...
        print(f"[agentic_retrieval] DEBUG: API call successful. Result type: {type(result)}")
        print(f"[agentic_retrieval] DEBUG: Result has response: {hasattr(result, 'response')}")
        if hasattr(result, 'response'):
            print(f"[agentic_retrieval] DEBUG: Response length: {len(result.response) if result.response else 0}")
    except HttpResponseError as err:
        print(f"[agentic_retrieval] DEBUG: HttpResponseError occurred: {err}")
        ...
        if (
            ...
            print(f"[agentic_retrieval] DEBUG: Retrying without target_index_params")
            req.target_index_params = None  # remove the conflicting override
            result = ka_client.knowledge_retrieval.retrieve(retrieval_request=req)
            print(f"[agentic_retrieval] DEBUG: Retry successful. Result type: {type(result)}")
        else:
    ...
    for msg in result.response:
        print(f"[agentic_retrieval] DEBUG: Processing message: {type(msg)}")
        for c in getattr(msg, "content", []):
            print(f"[agentic_retrieval] DEBUG: Processing content: {type(c)}")
            print(f"[agentic_retrieval] DEBUG: Content text: {getattr(c, 'text', 'NO_TEXT')[:200]}...")
            chunk = {
    ...
    print(f"[agentic_retrieval] DEBUG: Total chunks found: {len(chunks)}")
    print(f"[agentic_retrieval] DEBUG: Chunks preview: {chunks[:2] if chunks else 'NO_CHUNKS'}")
...
def run_streamlit_ui() -> None:
    ...
    with tab_test:
        ...
        try:
            ...
            st.session_state.available_indexes = available_indexes
        except Exception as e:
            st.error(f"Error loading indexes: {str(e)}")
            available_indexes = st.session_state.get('available_indexes', [])
        
        if not available_indexes:
            st.warning("⚠️ No indexes found. Please create an index first in the 'Create Index' or 'SharePoint Index' tabs.")
            st.stop()
        ...
        with col1:
            ...
            current_index = 0
            if st.session_state.selected_index and st.session_state.selected_index in available_indexes:
                current_index = available_indexes.index(st.session_state.selected_index)
            
            selected_index = st.selectbox(
                "Choose Index for Retrieval Testing",
                options=available_indexes,
                index=current_index,
                help="Select the search index to test retrieval against"
            )
        ...
        with col2:
            if st.button("🔄 Refresh Indexes"):
                # Force refresh of index list
                try:
                    fresh_indexes = [idx.name for idx in root_index_client.list_indexes()]
                    st.session_state.available_indexes = fresh_indexes
                    st.success("✅ Indexes refreshed")
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed to refresh: {str(e)}")
        
        # Show selected index status
        if st.session_state.selected_index:
            st.success(f"✅ Selected Index: **{st.session_state.selected_index}**")
            
            # Validate the selected index
            try:
                search_client, _ = init_search_client(st.session_state.selected_index)
                if search_client is None:
                    st.error(f"❌ Could not initialize search client for index '{st.session_state.selected_index}'. Please check if the index exists.")
                ...
                try:
                    ...
                    if doc_count == 0:
                        st.warning(f"⚠️ Index '{st.session_state.selected_index}' exists but contains no documents.")
                        st.info("Please index some documents first using the SharePoint or Upload tabs.")
                    else:
                        st.info(f"✅ Index contains {doc_count:,} documents ready for search.")
                except Exception as e:
                    st.warning(f"⚠️ Could not check document count: {str(e)}")
                    
            except Exception as e:
                st.error(f"❌ Error validating index: {str(e)}")
                return
            ...
            with col2:
                top_k = st.number_input("Top K", min_value=1, max_value=20, value=st.session_state.get('top_k', 5))
            ...
            with st.expander("💡 Sample Queries"):
                ...
                if hasattr(st.session_state, 'sample_query'):
                    test_query = st.session_state.sample_query
                    del st.session_state.sample_query
            
            # Search method and display mode selection
            col1, col2 = st.columns(2)
            with col1:
                search_method = st.radio("Search Method", ["Direct Search", "Agentic Search (Knowledge Agent)"], horizontal=True)
            with col2:
                display_mode = st.radio("Display Mode", ["Simple", "Debug (Detailed)"], horizontal=True)
            
            # Show explanation of search methods
            if search_method == "Direct Search":
                st.info("🔍 **Direct Search**: Raw search results showing individual document chunks")
            else:
                st.info("🤖 **Agentic Search**: Intelligent retrieval using Knowledge Agent for consolidated, contextual results")
                
                # Dynamically determine agent name based on selected index
                selected_index = st.session_state.selected_index
                agent_name = f"{selected_index}-agent"
                
                st.caption(f"Will use Knowledge Agent: `{agent_name}`")
                
                # Check if agent exists
                try:
                    agents_list = root_index_client.list_agents()
                    agent_names = [agent.name for agent in agents_list]
                    if agent_name not in agent_names:
                        st.warning(f"⚠️ Knowledge Agent `{agent_name}` not found. Available agents: {', '.join(agent_names)}")
                        st.info("💡 You may need to create the agent for this index in the 'Manage Index' tab.")
                except Exception as e:
                    st.warning(f"Could not check agent status: {e}")
                ...
                with st.spinner("Searching..."):
                    try:
                        ...
                        with st.expander("🔧 Search Parameters", expanded=False):
                            ...
                            st.write(f"**Display Mode:** `{display_mode}`")
                            
                            # Show index schema for debugging
                            try:
                                index_def = root_index_client.get_index(st.session_state.selected_index)
                                field_names = [field.name for field in index_def.fields]
                                st.write(f"**Available Fields:** `{', '.join(field_names)}`")
                            except Exception as schema_err:
                                st.write(f"**Schema Error:** `{str(schema_err)}`")
                        
                        # Execute search based on selected method
                        if search_method == "Direct Search":
                            # Direct search (raw Azure AI Search)
                            ...
                            # Debug: Show raw results count
                            st.info(f"🔍 Retrieved {len(search_results)} direct search results")
                            
                        else:
                            # Agentic search using Knowledge Agent
                            # Use dynamic agent name based on selected index
                            selected_index = st.session_state.selected_index
                            agent_name = f"{selected_index}-agent"
                            
                            try:
                                # Use agentic_retrieval function for intelligent search
                                messages = [{"role": "user", "content": test_query}]
                                # Let the knowledge agent use its own configured index (don't force an index)
                                agentic_response = agentic_retrieval(agent_name, None, messages)
                                
                                # Parse the JSON response from agentic retrieval
                                import json
                                agentic_results = json.loads(agentic_response)
                                
                                # Convert agentic results to search result format for display consistency
                                search_results = []
                                for i, chunk in enumerate(agentic_results[:top_k]):
                                    search_results.append({
                                        "id": chunk.get("ref_id", i),
                                        "content": chunk.get("content", ""),
                                        "page_chunk": chunk.get("content", ""),
                                        "source_file": chunk.get("source_file", "Unknown"),
                                        "page_number": chunk.get("page_number", None),
                                        "url": chunk.get("url", ""),
                                        "@search.score": chunk.get("score", 0.0) or 1.0,  # Fallback for missing scores
                                        "doc_key": chunk.get("doc_key", ""),
                                        "_agentic": True  # Flag to indicate this came from agentic search
                                    })
                                
                                # Debug: Show agentic results count
                                st.info(f"🤖 Retrieved {len(search_results)} agentic search results from Knowledge Agent")
                                
                            except Exception as agentic_err:
                                st.error(f"❌ Agentic search failed: {str(agentic_err)}")
                                st.markdown("**Falling back to direct search...**")
                                ...
                                search_results = list(results)
                                st.info(f"🔍 Retrieved {len(search_results)} direct search results (fallback)")
                        ...
                        if search_results:
                            ...
                            if display_mode == "Simple":
                                ...
                                for i, result in enumerate(search_results):
                                    # Show different icon for agentic vs direct results
                                    result_icon = "🤖" if result.get("_agentic") else "📄"
                                    result_type = "Agentic" if result.get("_agentic") else "Direct"
                                    
                                    with st.expander(f"{result_icon} {result_type} Result {i+1}: {result.get('source_file', 'Unknown')} (Score: {result['@search.score']:.3f})", expanded=False):
                                        ...
                                        if len(content) > preview_length:
                                            ...
                                            # Add show full content toggle
                                            show_full_key = f"show_full_{i}"
                                            if st.button(f"📖 Show Full Content", key=show_full_key):
                                                st.text_area(f"Full Content - Result {i+1}", content, height=300, key=f"full_content_{i}")
                                        else:
                                            st.write(content)
                                        ...
                                        col1, col2 = st.columns(2)
                                        with col1:
                                            st.markdown(f"**📁 Source:** `{result.get('source_file', 'Unknown')}`")
                                            if result.get('page_number'):
                                                st.markdown(f"**📄 Page:** `{result.get('page_number')}`")
                                        
                                        with col2:
                                            if result.get('url'):
                                                st.markdown(f"**🔗 URL:** [{result.get('url')}]({result.get('url')})")
                                            st.markdown(f"**⭐ Score:** `{result['@search.score']:.3f}`")
                                            if result.get("_agentic"):
                                                st.markdown(f"**🤖 Source:** `Knowledge Agent`")
                            else:
                                # Debug (Detailed) display mode
                                for i, result in enumerate(search_results):
                                    # Show different icon for agentic vs direct results
                                    result_icon = "🤖" if result.get("_agentic") else "🔍"
                                    result_type = "Agentic" if result.get("_agentic") else "Direct"
                                    
                                    with st.expander(f"{result_icon} {result_type} Debug Result {i+1}: {result.get('source_file', 'Unknown')} (Score: {result['@search.score']:.3f})", expanded=True):
                                        # Full content
                                        content = result.get('content', result.get('page_chunk', 'No content'))
                                        ...
                                        import time
                                        unique_key = f"debug_content_{i}_{int(time.time() * 1000) % 10000}"
                                        st.text_area(
                                            f"Content from {result_type} Result {i+1}", 
                                            content, 
                                            height=200, 
                                            key=unique_key,
                                            help="Full document content for this search result"
                                        )
                                        ...
                                        with col1:
                                            ...
                                            st.write(f"• **Source File:** `{result.get('source_file', 'Unknown')}`")
                                            st.write(f"• **Page Number:** `{result.get('page_number', 'N/A')}`")
                                            st.write(f"• **Document ID:** `{result.get('id', 'Unknown')}`")
                                            st.write(f"• **Chunk ID:** `{result.get('chunk_id', 'N/A')}`")
                                            if result.get('title'):
                                                st.write(f"• **Title:** `{result.get('title')}`")
                                            if result.get("_agentic"):
                                                st.write(f"• **Search Type:** `🤖 Knowledge Agent`")
                                            else:
                                                st.write(f"• **Search Type:** `🔍 Direct Search`")
                                        
                                        with col2:
                                            st.markdown("**🔍 Search Metadata:**")
                                            st.write(f"• **Search Score:** `{result['@search.score']:.6f}`")
                                            st.write(f"• **URL:** `{result.get('url', 'No URL')}`")
                                            st.write(f"• **Last Modified:** `{result.get('last_modified', 'Unknown')}`")
                                            st.write(f"• **File Size:** `{result.get('size', 'Unknown')}`")
                                            if result.get('file_type'):
                                                st.write(f"• **File Type:** `{result.get('file_type')}`")
                                            if result.get('doc_key'):
                                                st.write(f"• **Doc Key:** `{result.get('doc_key')}`")
                                        ...
                                        # Raw result data
                                        with st.expander("🔧 Raw Result Data (JSON)", expanded=False):
                                            st.json(dict(result), expanded=False)
                        else:
                            st.info("No results found for this query.")
                            st.markdown("**Suggestions:**")
                            st.markdown("• Try different search terms")
                            st.markdown("• Check if the index contains relevant documents")
                            st.markdown("• Verify the index name is correct")
                            
                    except Exception as e:
                        st.error(f"Search failed: {str(e)}")
                        if display_mode == "Debug (Detailed)":
                            st.markdown("**Error Details:**")
                            st.code(str(e))
                            st.markdown("**Troubleshooting:**")
                            st.markdown("• Check if the search service is running")
                            st.markdown("• Verify the index exists and has documents")
                            st.markdown("• Check your authentication credentials")
